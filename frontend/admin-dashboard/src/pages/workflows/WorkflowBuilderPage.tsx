...
export function WorkflowBuilderPage() {
  ...
  const [conditions, setConditions] = useState<Condition[]>(
    isNew ? [] : [{ field: 'outcome', operator: 'equals', value: 'completed' }]
  );
  const [actions, setActions] = useState<Action[]>(
    isNew ? [] : [{ type: 'create_lead', config: { source: 'inbound-call' } }]
  );

  const addCondition = () => {
    setConditions([...conditions, { field: 'outcome', operator: 'equals', value: '' }]);
  };
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/workflows')} className="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100">
      ...
      <Card>
        <CardHeader
          title="Conditions"
          subtitle="Only run when these conditions are met (optional)"
          action={
        ...
        />
        {conditions.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-4">No conditions - workflow will run on every trigger</p>
        ) : (
          <div className="space-y-3">
            {conditions.map((cond, idx) => (
              <div key={idx} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                <span className="text-xs text-gray-400 font-medium w-6">{idx === 0 ? 'IF' : 'AND'}</span>
                <select
                  value={cond.field}
                ...
                <input
                  ...
                  className="text-sm border border-gray-200 rounded-lg px-3 py-2 bg-white flex-1"
                />
                <button onClick={() => removeCondition(idx)} className="p-1.5 text-gray-400 hover:text-danger-600">
      ...
      <Card>
        <CardHeader
          title="Actions"
          subtitle="What should happen when conditions are met?"
          action={
        ...
        />
        {actions.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-4">Add at least one action</p>
        ) : (
          <div className="space-y-3">
            {actions.map((action, idx) => (
              <div key={idx} className="flex items-start gap-3 p-4 bg-gray-50 rounded-lg">
                <span className="text-xs text-gray-400 font-medium w-8 mt-2.5">THEN</span>
                ...
                <button onClick={() => removeAction(idx)} className="p-1.5 text-gray-400 hover:text-danger-600 mt-2">
                  <Trash2 className="h-4 w-4" />
                </button>
              </div>
            ))}
