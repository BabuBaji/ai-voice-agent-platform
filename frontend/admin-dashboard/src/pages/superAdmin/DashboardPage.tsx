...
function StatLink({
  ...
  return (
    <Link to={to} className="group bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-slate-300 transition-all relative overflow-hidden">
      <div className="flex items-start justify-between">
        ...
        <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${accentMap[accent]} flex items-center justify-center shadow-md flex-shrink-0`}>
      ...
      <p className="text-[11px] text-slate-400 mt-2 inline-flex items-center gap-1 group-hover:text-amber-700">
...
function PanelEmpty({ msg = 'No data yet' }: { msg?: string }) {
  return <div className="h-48 flex items-center justify-center text-xs text-slate-400">{msg}</div>;
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Live overview · auto-refreshes every 60s · click anything to drill in</p>
        </div>
        <div className="flex items-center gap-2 text-xs text-slate-500">
          ...
          <button
            onClick={() => load(true)}
            disabled={refreshing}
            className="ml-2 inline-flex items-center gap-1 px-2.5 py-1 rounded-md border border-slate-200 hover:border-amber-400 hover:text-amber-700 text-slate-600 transition disabled:opacity-50"
            title="Refresh now"
          >
